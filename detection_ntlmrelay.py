...
GLOBAL_computers = {'10.0.0.20': 'WIN10-CLIENT',
...
def get_network_interfaces() -> List:
    ...
    for interface in interfaces:
        interface_out = subprocess.check_output(
        ...
        interfaces_output = interfaces_output[5:interfaces_output.find(
            ">") + 1]
        interfaces_output = interfaces_output.replace(
            ":", "").replace("<", "").replace(">", "")
...
def detect_ntlmrelayx(challenge):
    if challenge == "41:41:41:41:41:41:41:41":
        return True
    else:
        return False
...
def dectection_ntlm_traffic(packet):
    ...
    if ntlmtype == 'negotiate':
        ...
        if detect_ntlmrelayx(challenge):
            add_alert("ntlmrelayx.py", source, 'NA', layer._layer_name)
        #     #print("ntlmrelayx.py is gesignaleerd in het network op ip", source)
        if not detect_double_challenge(challenge, source):
            add_alert("Relay", source, GLOBAL_computers.get(dest), layer._layer_name)
    elif ntlmtype == 'auth':
        ...
        if not valid_computer:
            add_alert("Authentication", source, GLOBAL_computers.get(dest), layer._layer_name)
...
def capture_live_analysis(chosen_interface):
    ...
    capture = pyshark.LiveCapture(
        interface=chosen_interface, display_filter='ntlmssp')
